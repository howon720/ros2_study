...
CmdPublisher::CmdPublisher() : Node("cmd_publisher"), goal_x(0.0), goal_y(0.0), goal_received(false)  {
  ...
  // timer_tf = this->create_wall_timer(
  //       50ms, [this]() {
  //           RCLCPP_INFO(this->get_logger(), "timer_tf_callback is active.");
  //           timer_tf_callback();
  //       });
...
void CmdPublisher::timer_cmd_callback() {
  ...
  ///////////////////////////////////////////A* 경로 생성만 잘 되는 코드////////////////////////////////
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(now - last_a_star_time).count() >= 1) {
      ...
      //       } else {
      //           path = std::move(new_path);  // 경로 업데이트
      //           visualize_path(path);       // 경로 시각화
                
      //       }
      //   }).detach();

      //   last_a_star_time = now;  // 마지막 계산 시간 갱신
      // }
...
//           // 시작점
//           octomap::point3d start = (path.size() > 3) ? path[3] : octomap::point3d(x, y, z);
//           auto new_path = generate_path_a_star(start, goal);
      
//           if (new_path.empty() || new_path.size() < 3) {
//               RCLCPP_WARN(this->get_logger(), "Path not valid.");
//               return;
//           }
      
//           // === 경로 변화량이 작으면 업데이트 안 함 ===
//           if (path.size() >= 4) {
//               float dx = path[3].x() - new_path[0].x();
//               float dy = path[3].y() - new_path[0].y();
//               float dist = std::sqrt(dx * dx + dy * dy);
//               if (dist < 0.1) {  // 거의 같으면 무시 (이 값은 조절 가능)
//                   RCLCPP_INFO(this->get_logger(), "Path similar. Skipping update.");
//                   return;
//               }
//           }
      
//           // 경로 업데이트
//           std::vector<octomap::point3d> final_path;
//           if (path.size() > 0) final_path.push_back(path[0]); // 유지
//           final_path.push_back(new_path[0]);
//           final_path.push_back(new_path[1]);
//           final_path.insert(final_path.end(), new_path.begin() + 2, new_path.end());
      
//           path = std::move(final_path);
//           visualize_path(path);
//       }).detach();
...
std::vector<octomap::point3d> CmdPublisher::generate_path_a_star(const octomap::point3d& start, const octomap::point3d& goal) {
 struct Node {
  ...
  // //heuristic 방향을 유도하기 위한 가상 시작점 (path[3])
  // octomap::point3d virtual_start = (this->path.size() > 3) ? this->path[3] : start;

  // //현재 위치에서 경로 시작, 방향성은 virtual_start 기준
  // Node start_node = {start, 0.0f, (goal - virtual_start).norm(), nullptr};
...
void CmdPublisher::follow_path(const std::vector<octomap::point3d>& path) { 
    ...
    // // PD 게인 (필요 시 조정)
    // const float Kp_dist = 2.3f;  //3.   
    // const float Kd_dist = 0.4f;  //0.5 
    // const float Kp_angle = 1.2f;   // 각도가 잘 안 줄면 값 증가  1.2
    // const float Kd_angle = 0.25f;  //0.25

    // // 속도 제한
    // const float max_linear_speed  = 0.7f;   //1.0
    // const float max_angular_speed = 1.2f;  // 회전 속도 좀 높여볼 수 있음 1.5
